(function () {
  ...
  function loadSample() {
    ...
    sample.textContent = "正在载入示例…";
    try {
      var sampleData = document.getElementById("sample-data");
      if (!sampleData) throw new Error("未找到内置示例数据。");
      var data = JSON.parse(sampleData.textContent.replace(/^\+/gm, ""));
      if (!Array.isArray(data.defects) || !data.defects.length) throw new Error("内置示例中没有缺陷记录。");
      ...
      data.defects.forEach(function (defect, index) {
        ...
        defect.reference_images = [sampleRoot + "location_maps_web/" + imageId + "_map.jpg", sampleRoot + "defects/" + imageId + ".jpg", sampleRoot + "defect_masks/" + imageId + "_mask.png", sampleRoot + "defect_masks/" + imageId + "_vis.jpg"];
        ...
        defect.capture_height = examples.altitude; defect.gps_latitude = examples.latitude; defect.gps_longitude = examples.longitude; defect.pedestrian_risk = "否"; defect.severity_reason = examples.severity; defect.treatment_advice = sampleStyleAdvice(defect.analysis_input); defect.expert_conclusion = defect.expert_conclusion || {}; defect.expert_conclusion.text = examples.basis;
      ...
      readiness.textContent = "示例已载入，等待生成预览";
    } catch (error) {
      checks.innerHTML = '<div class="check bad">内置示例载入失败：' + error.message + "</div>";
      fileList.innerHTML = "<ul><li>示例载入失败：" + error.message + "</li></ul>";
      generate.disabled = true;
      setPhase(1);
    } finally {
      sample.disabled = false;
      sample.textContent = "载入 Building_76 示例";
    }
  }
